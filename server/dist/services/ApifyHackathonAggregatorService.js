...
class ApifyHackathonAggregatorService {
     ...
     */
    static async purgeSyntheticHackathons() {
        const demoKeywords = [
     ...
     */
    static async syncAllPlatforms(customToken, forceFreshRun = false) {
        ...
        try {
            ...
            const startRes = await fetch(`https://api.apify.com/v2/acts/${this.aggregatorActorId}/runs?token=${token}&memory=1024`, {
                method: 'POST',
            ...
            if (rawItems.length === 0) {
                ...
                const userRunsRes = await fetch(`https://api.apify.com/v2/acts/${this.aggregatorActorId}/runs?status=SUCCEEDED&limit=1&desc=true&token=${token}`);
                if (userRunsRes.ok) {
                    ...
                    if (fallbackDatasetId) {
                        ...
                        if (fallbackRes.ok) {
                            rawItems = (await fallbackRes.json());
                        }
                    }
                }
            }
            ...
            for (const item of rawItems) {
                ...
                const slug = cleanTitle.slice(0, 45).replace(/\s+/g, '-') +
                    '-' +
    ...
    static normalizePlatform(source) {
        const s = (source || '').toLowerCase().trim();
        if (s.includes('devpost'))
            return 'Devpost';
        if (s.includes('mlh'))
            return 'MLH';
        if (s.includes('dorahacks') || s.includes('dora'))
            return 'DoraHacks';
        if (s.includes('devfolio'))
            return 'Devfolio';
        if (s.includes('unstop'))
            return 'Unstop';
        return 'Devpost';
    }
    static inferPlatformFromUrl(url) {
        const u = url.toLowerCase();
        if (u.includes('devpost.com'))
            return 'Devpost';
        if (u.includes('mlh.io') || u.includes('mlh.com'))
            return 'MLH';
        if (u.includes('dorahacks.io'))
            return 'DoraHacks';
        if (u.includes('devfolio.co'))
            return 'Devfolio';
        if (u.includes('unstop.com'))
            return 'Unstop';
        return 'Devpost';
    }
    static parseDateRange(dateStr, explicitStart, explicitEnd) {
        const now = new Date();
    ...
    static inferTheme(title, themes) {
    ...
    static inferDepartment(theme) {
        if (theme === 'AI/ML' || theme === 'Data Science')
            return 'AI & DS';
        if (theme === 'IoT')
            return 'ECE';
        if (theme === 'Blockchain' || theme === 'Cybersecurity' || theme === 'Fintech')
            return 'IT';
        return 'CSE';
    }
    static getLogoForPlatform(platform) {
        const map = {
    ...
    static getBannerForPlatform(platform, theme) {
        const banners = [
...
exports.ApifyHackathonAggregatorService = ApifyHackathonAggregatorService;
