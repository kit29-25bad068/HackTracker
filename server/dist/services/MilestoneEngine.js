...
exports.MilestoneEngine = void 0;
...
class MilestoneEngine {
     ...
     */
    static async evaluateMilestones(userId) {
        ...
        if (!user)
            return [];
        const existingMilestoneCodes = new Set(user.milestones.map((m) => m.milestone.code));
        const allMilestones = await prisma.milestone.findMany();
        const newlyAchievedCodes = [];
        const totalProjects = user.projects.length;
        const winnerProjects = user.projects.filter((p) => p.status === 'Winner').length;
        const verifiedCertificates = user.projects.filter((p) => p.isVerified && p.certificate?.status === 'VERIFIED').length;
        const ruleChecks = {
...
exports.MilestoneEngine = MilestoneEngine;
