...
// 4. ENDORSE A SKILL
router.post('/:userSkillId/endorse', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (existingEndorsement) {
            ...
            await TrustScoreService_1.TrustScoreService.recalculateUserScore(userSkill.userId);
        ...
        await TrustScoreService_1.TrustScoreService.recalculateUserScore(userSkill.userId);
        const unlockedBadges = await BadgeEngine_1.BadgeEngine.evaluateBadges(userSkill.userId);
...
// 5. DELETE SKILL
router.delete('/:userSkillId', auth_1.requireAuth, async (req, res) => {
    try {
        const userSkillId = req.params.userSkillId;
        const userId = req.user.id;
        const userSkill = await prisma.userSkill.findUnique({
        ...
        await TrustScoreService_1.TrustScoreService.recalculateUserScore(userId);
