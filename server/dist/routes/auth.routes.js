...
router.post('/signup', async (req, res) => {
    try {
        ...
        await TrustScoreService_1.TrustScoreService.recalculateLeaderboardRanks();
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (user.is2faEnabled && user.twoFactorSecret) {
            const secret = process.env.JWT_SECRET || 'fallback_secret_key';
            const tempToken = jsonwebtoken_1.default.sign({ userId: user.id, isTemp2FA: true }, secret, { expiresIn: '10m' });
            res.json({
                requires2FA: true,
                tempToken,
...
router.post('/2fa/verify-login', async (req, res) => {
    try {
        ...
        const secret = process.env.JWT_SECRET || 'fallback_secret_key';
        const decoded = jsonwebtoken_1.default.verify(tempToken, secret);
        ...
        let isCodeValid = otplib_1.authenticator.verify({
            token: code.trim(),
        ...
        if (!isCodeValid && user.backupCodes) {
            try {
                const backupCodesList = JSON.parse(user.backupCodes);
...
router.post('/2fa/generate', auth_1.requireAuth, async (req, res) => {
    try {
        const user = req.user;
...
router.post('/2fa/enable', auth_1.requireAuth, async (req, res) => {
    try {
...
router.post('/2fa/disable', auth_1.requireAuth, async (req, res) => {
    try {
