...
router.post('/', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await BadgeEngine.evaluateBadges(user.id);
...
router.post('/:id/accept', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await TrustScoreService.recalculateTeamScore(id);
...
// 7. REMOVE MEMBER
router.delete('/:id/members/:targetUserId', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const id = req.params.id as string;
