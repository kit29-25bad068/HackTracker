...
import { TRUST_SCORE_CONFIG, POINTS_CONFIG } from '../config/constants';
...
export class TrustScoreService {
   ...
   */
  static async recalculateUserScore(userId: string): Promise<{
    trustScore: number;
    points: number;
    finalScore: number;
    currentRank: number;
    ...
    let calculatedTrust = TRUST_SCORE_CONFIG.BASE_SCORE;
    ...
    for (const project of user.projects) {
      // Points calculation
      calculatedPoints += POINTS_CONFIG.PROJECT_SUBMISSION;

      if (project.status === 'Winner') {
        calculatedPoints += POINTS_CONFIG.PROJECT_WIN;
        winsCount++;
      }

      if (project.teamId) {
        calculatedPoints += POINTS_CONFIG.TEAM_PROJECT_BONUS;
      }

      // Trust score calculation
      if (project.isVerified && project.certificate?.status === 'VERIFIED') {
        calculatedTrust += TRUST_SCORE_CONFIG.OCR_VERIFIED_BONUS;
        verifiedCount++;
      } else {
        calculatedTrust += TRUST_SCORE_CONFIG.MANUAL_PROJECT_BONUS;
        manualCount++;
      }
    }

    // Badge points bonus
    calculatedPoints += user.badges.length * POINTS_CONFIG.BADGE_EARNED;
    ...
    calculatedPoints += totalEndorsements * POINTS_CONFIG.SKILL_ENDORSEMENT;

    // Consecutive verified bonus
    if (verifiedCount >= 3) {
      calculatedTrust += TRUST_SCORE_CONFIG.CONSECUTIVE_VERIFIED_BONUS;
    }

    // Clamp trust score between 0% and 100%
    calculatedTrust = Math.min(
      TRUST_SCORE_CONFIG.MAX_SCORE,
      Math.max(TRUST_SCORE_CONFIG.MIN_SCORE, calculatedTrust)
    );

    // Final score formula: Final Score = Points * (Trust Score / 100)
    const finalScore = parseFloat(
      (calculatedPoints * (calculatedTrust / 100.0)).toFixed(2)
    );
   ...
   */
  static async recalculateTeamScore(teamId: string): Promise<void> {
