  ...
  public static async syncLiveUnstopHackathons(customToken?: string): Promise<{
    ...
    try {
      ...
      const startRes = await fetch(
        `https://api.apify.com/v2/acts/${this.actorId}/runs?token=${token}&memory=1024`,
        {
          method: 'POST',
      ...
      if (datasetId) {
        ...
        if (itemsRes.ok) {
          items = (await itemsRes.json()) as ApifyUnstopItem[];
        }
      }
      ...
      if (items.length === 0) {
        ...
        const userRunsRes = await fetch(
          `https://api.apify.com/v2/acts/${this.actorId}/runs?status=SUCCEEDED&limit=1&desc=true&token=${token}`
        );
        if (userRunsRes.ok) {
          ...
          if (fallbackDatasetId) {
            ...
            if (fallbackRes.ok) {
              items = (await fallbackRes.json()) as ApifyUnstopItem[];
            }
          }
        }
      }
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        // Infer Department
        let department = 'CSE';
        if (theme === 'AI/ML') department = 'AI & DS';
        else if (theme === 'IoT') department = 'ECE';
        else if (theme === 'Blockchain' || theme === 'Cybersecurity') department = 'IT';
